...
async function mirrorDesks(store: MetadataStore | null, items: import("@neo-bot/contracts").Desk[]): Promise<void> {
  if (!store) {
    return;
  }
  const remote = await store.loadDesks();
...
async function mirrorExperts(store: MetadataStore | null, items: import("@neo-bot/contracts").Expert[]): Promise<void> {
  if (!store) {
    return;
  }
  const remote = await store.loadExperts();
...
async function mirrorPluginInstalls(
  ...
  items: import("@neo-bot/contracts").PluginInstall[],
): Promise<void> {
  if (!store) {
    return;
  }
...
async function mirrorProjects(store: MetadataStore | null, items: import("@neo-bot/contracts").Project[]): Promise<void> {
  if (!store) {
    return;
  }
  const remote = await store.loadProjects();
