...
import type { Run } from "@neo-bot/contracts";
...
test("writeRecalledMemory writes MEMORY.md from search hits", async () => {
  ...
  setMem0FetchForTests(async (url) => {
    const hits = [{ id: "m1", memory: "用 pnpm", score: 0.9, metadata: { source: "manual", kind: "coding" } }];
    if (String(url).includes("/search") || String(url).includes("/memories?")) {
  ...
  try {
    ...
    assert.match(text, /tendencies/);
    assert.match(text, /not veto power/);
    assert.doesNotMatch(text, /Follow them unless/);
  } finally {
...
test("writeUserRules writes USER.md and disabled memory skips MEMORY.md", async () => {
  ...
  const previousRuntime = process.env.WORKER_RUNTIME;
  process.env.WORKER_RUNTIME = "none";
  process.env.RUNS_DIR = mkdtempSync(path.join(tmpdir(), "neo-mem-rules-"));
  process.env.MEM0_URL = "http://mem0.test";
  process.env.MEM0_API_KEY = "m0sk_test";
