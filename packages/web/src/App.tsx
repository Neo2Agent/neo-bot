...
} from "@neo-bot/contracts/transcript";
import type { RunEvent, TranscriptMessage, TranscriptSnapshot } from "@neo-bot/contracts/events";
import { decodeExpertPick, encodeExpertPick, expertPickerLabel, type Expert, type ExpertPick, type ExpertTeam } from "@neo-bot/contracts/expert";
import { isRemoteControlTarget, type AgentMode, type ImageRef, type Run } from "@neo-bot/contracts/run";
...
import type { Project } from "@neo-bot/contracts/project";
import type { ProjectAsset } from "@neo-bot/contracts/project-asset";
import { BUNDLED_RECIPES, recipeById, type IntentCapsule, type Recipe } from "@neo-bot/contracts/recipe";
import { pluginPickerLabel, type PluginCatalogItem } from "@neo-bot/contracts/plugin";
import { parseProjectHash, projectHashHref } from "./project-route.js";
import { InboxBell } from "./components/InboxBell";
import { BuddyHome, BuddyPlusSheet, buddySkillsFromRecipes, type BuddyPlusAction } from "@neo-bot/ui";
...
import {
  ...
  parseContextUsage,
  resolveModelLimits,
} from "@neo-bot/contracts/context-usage";
...
function hashAutomations(): boolean {
  if (!WEB_V1.automationsUi) return false;
...
export function App() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      const remembered = WEB_V1.deskUi
        ? ((await deskBridge()?.getTarget().catch(() => undefined)) ?? readLastTarget())
        : { kind: "cloud" as const };
      if (cancelled) return;
      if (remembered) {
