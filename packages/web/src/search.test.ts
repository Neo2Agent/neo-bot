...
import type { TranscriptMessage } from "@neo-bot/contracts/events";
import { searchTranscript, userQuestions } from "./search.js";
...
test("searchTranscript matches text and skips empty queries", () => {
  ...
  assert.deepEqual(searchTranscript(messages, "鉴权").map((item) => item.id), ["1"]);
});

test("userQuestions keeps only user turns", () => {
  const messages = [msg("1", "user", "一"), msg("2", "assistant", "二"), msg("3", "setup", "三")];
  assert.deepEqual(userQuestions(messages).map((item) => item.id), ["1"]);
});
